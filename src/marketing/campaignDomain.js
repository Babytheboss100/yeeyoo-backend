...
export const CAMPAIGN_STATUSES = Object.freeze(['draft', 'planned', 'active', 'paused', 'completed', 'archived'])
const transitions = Object.freeze({ draft: ['planned', 'archived'], planned: ['draft', 'active', 'archived'], active: ['paused', 'completed'], paused: ['active', 'completed', 'archived'], completed: ['archived'], archived: [] })
...
export function createCampaign(input, { id = crypto.randomUUID(), now = new Date().toISOString() } = {}) {
  ...
  const context = { audience: input.audience || null, offer: input.offer || null, budget: input.budget ?? null, startDate: input.startDate || null, endDate: input.endDate || null, channels: [...new Set(input.channels || [])] }
  return { id, userId: input.userId, projectId: input.projectId, schemaVersion: CAMPAIGN_SCHEMA_VERSION, name, objective: String(input.objective ?? '').trim() || null, status: 'draft', context, createdAt: now, updatedAt: now }
}
