...
export function createArtifactRecord(input, { id = crypto.randomUUID(), now = new Date().toISOString() } = {}) {
  ...
  return { id, rootId: input.rootId || id, parentId: input.parentId || null, userId: input.userId, projectId: input.projectId, campaignId: input.campaignId || null, type: input.type,
...
const decode = row => row && ({ id: row.id, rootId: row.root_id || row.id, parentId: row.parent_id || null, userId: row.user_id, projectId: row.project_id, campaignId: row.campaign_id, type: row.type, schemaVersion: row.schema_version, artifactVersion: row.artifact_version, status: row.status, purpose: row.purpose, channel: row.channel, content: row.content, provenance: row.provenance, provider: row.provider, model: row.model, approvedAt: row.approved_at, createdAt: row.created_at, updatedAt: row.updated_at })
export async function saveArtifact(input, db = pool) { const a = createArtifactRecord(input); const { rows } = await db.query(`INSERT INTO marketing_artifacts (id,root_id,parent_id,user_id,project_id,campaign_id,type,schema_version,artifact_version,status,purpose,channel,content,provenance,provider,model) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9,'draft',$10,$11,$12,$13,$14,$15) RETURNING *`, [a.id,a.rootId,a.parentId,a.userId,a.projectId,a.campaignId,a.type,a.schemaVersion,a.artifactVersion,a.purpose,a.channel,JSON.stringify(a.content),JSON.stringify(a.provenance),a.provider,a.model]); return decode(rows[0]) }
...
export async function getArtifact({ id,userId,projectId,db=pool }) { const {rows}=await db.query('SELECT * FROM marketing_artifacts WHERE id=$1 AND user_id=$2 AND project_id=$3',[id,userId,projectId]); return decode(rows[0]) || null }
export async function createArtifactVersion({ source,userId,projectId,content,provider='manual-edit',model='human',db=pool }) {
  if (!source || source.userId !== userId || source.projectId !== projectId) return null
  const latest=await db.query('SELECT COALESCE(MAX(artifact_version),0) AS version FROM marketing_artifacts WHERE user_id=$1 AND project_id=$2 AND root_id=$3',[userId,projectId,source.rootId])
  return saveArtifact({ userId,projectId,rootId:source.rootId,parentId:source.id,campaignId:source.campaignId,type:source.type,artifactVersion:Number(latest.rows[0].version)+1,purpose:source.purpose,channel:source.channel,content,provenance:{...source.provenance,derivedFromArtifactId:source.id},provider,model },db)
}
