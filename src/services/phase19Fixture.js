...
export async function seedPhase19Fixture(client){await assertPhase19Database(client);const f=PHASE19;await client.query('BEGIN');try{
  ...
  await client.query('DELETE FROM marketing_approval_decisions WHERE user_id=$1 AND project_id=$2 AND artifact_id IN (SELECT id FROM marketing_artifacts WHERE user_id=$1 AND project_id=$2 AND root_id=$3)',[f.userId,f.projectId,f.artifactRootId])
  await client.query('DELETE FROM posts WHERE user_id=$1 AND project_id=$2 AND artifact_id IN (SELECT id FROM marketing_artifacts WHERE user_id=$1 AND project_id=$2 AND root_id=$3)',[f.userId,f.projectId,f.artifactRootId])
